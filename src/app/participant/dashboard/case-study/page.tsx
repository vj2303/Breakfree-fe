...
const CaseStudyPageWithSearchParams = () => {
  ...
  // Calculate total time from scenarios and tasks (readTime + exerciseTime)
  const calculateTotalTime = (): number => {
    if (!activityData?.activityDetail) {
      return 0;
    }
    let total = 0;
    
    // Sum up all readTime and exerciseTime from scenarios
    if (activityData.activityDetail.scenarios) {
      total += activityData.activityDetail.scenarios.reduce((sum, scenario) => {
        return sum + (scenario.readTime || 0) + (scenario.exerciseTime || 0);
      }, 0);
    }
    
    // Sum up all readTime and exerciseTime from tasks
    if (activityData.activityDetail.tasks) {
      total += activityData.activityDetail.tasks.reduce((sum, task) => {
        return sum + (task.readTime || 0) + (task.exerciseTime || 0);
      }, 0);
    }
          ...
          {steps.map((step, idx) => (
            <div key={step} className="flex items-center gap-2 flex-shrink-0">
              <div className="flex flex-col items-center">
                <span className={`text-xs font-medium whitespace-nowrap ${currentStep === idx ? 'text-black font-semibold' : currentStep > idx ? 'text-gray-700' : 'text-gray-500'}`}>
                  {step}
                </span>
                <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mt-1.5 transition-all duration-200
                  ${currentStep === idx ? 'border-black bg-white' : 
                    currentStep > idx ? 'border-gray-700 bg-gray-100' : 
              ...
              {idx < steps.length - 1 && (
                <div className={`w-8 h-0.5 transition-all duration-200 ${
                  currentStep > idx ? 'bg-gray-700' : 'bg-gray-300'
                }`} />
              )}
            </div>
          ))}
